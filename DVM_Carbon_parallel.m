...
Glob_Aday = zeros([size(YRUN),P.n]); Glob_Anight = zeros([size(YRUN), P.n]); % [gC m^-3] Concentration of the organisms in each water layer at day and at night
Glob_Cday = zeros([size(YRUN),P.n]); Glob_Cnight = zeros([size(YRUN), P.n]);
Glob_Pday = zeros([size(YRUN),P.n]); Glob_Pnight = zeros([size(YRUN), P.n]);
Glob_Mday = zeros([size(YRUN),P.n]); Glob_Fnight = zeros([size(YRUN), P.n]);
Glob_Fday = zeros([size(YRUN),P.n]); Glob_Mnight = zeros([size(YRUN), P.n]);
Glob_Jday = zeros([size(YRUN),P.n]); Glob_Jnight = zeros([size(YRUN), P.n]);

Glob_AdayT = zeros(size(lat_coordTot,2),P.n); Glob_AnightT = zeros(size(lat_coordTot,2),P.n); %different format so that we can use them in the parfor loop
Glob_CdayT = zeros(size(lat_coordTot,2),P.n); Glob_CnightT = zeros(size(lat_coordTot,2),P.n);
Glob_PdayT = zeros(size(lat_coordTot,2),P.n); Glob_PnightT = zeros(size(lat_coordTot,2),P.n);
Glob_MdayT = zeros(size(lat_coordTot,2),P.n); Glob_FnightT = zeros(size(lat_coordTot,2),P.n);
Glob_FdayT = zeros(size(lat_coordTot,2),P.n); Glob_MnightT = zeros(size(lat_coordTot,2),P.n);
Glob_JdayT = zeros(size(lat_coordTot,2),P.n); Glob_JnightT = zeros(size(lat_coordTot,2),P.n);

Glob_A = zeros([size(YRUN),P.n,P.n]); % [-] Distribution of the different populations in the strategy space
Glob_C = zeros([size(YRUN),P.n,P.n]);
Glob_P = zeros([size(YRUN),P.n,P.n]);
Glob_M = zeros([size(YRUN),P.n,P.n]);
Glob_F = zeros([size(YRUN),P.n,P.n]);
Glob_J = zeros([size(YRUN),P.n,P.n]);


Glob_AT = zeros(size(lat_coordTot,2),P.n,P.n); %different format so that we can use them in the parfor loop
Glob_CT = zeros(size(lat_coordTot,2),P.n,P.n); 
Glob_PT = zeros(size(lat_coordTot,2),P.n,P.n);
Glob_MT = zeros(size(lat_coordTot,2),P.n,P.n);
Glob_FT = zeros(size(lat_coordTot,2),P.n,P.n);
Glob_JT = zeros(size(lat_coordTot,2),P.n,P.n);
...
Glob_FitAT = zeros(size(lat_coordTot,2),1);
Glob_FitCT = zeros(size(lat_coordTot,2),1);
Glob_FitPT = zeros(size(lat_coordTot,2),1);
Glob_FitMT = zeros(size(lat_coordTot,2),1);
