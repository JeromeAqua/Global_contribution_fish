...
P.O2 =  2*(1-tanh(max(0,(P.zi-100)/150))) + 2*P.zi/P.ZMAX; % [mgO2/L] Oxygen concentration in the water column
pO2func = @(z) (z<300).*(-0.0667*z+21)+(z>300).*(z<500).*(0.05*z-14)+(z>500)*11; %@(z) (z<400).*(-0.05*z+21)+(z>400).*(z<600).*(0.05*z-19)+(z>600)*11;
P.pO2 = 21*ones(size(P.zi)); %pO2func(P.zi); %min(21,P.O2./(0.381*exp(5.7018*(28-P.T)./(P.T+273.15)))/0.75); % [kPa] Partial pressure of oxygen in the water column
...
P.R  = 0.01*exp(-(P.zi-50).^2/30^2); %0.01*(1-tanh((P.zi-P.z0)/P.zm))/2; % [gC / m3] Resource concentration
P.D = 0.01*min(50^-0.86,(P.zi-50).^-0.86)/50^(-0.86); % [gC / m^3] Detritus concentration - nothing eats it for now
...
%%%%%%%%%%% ABUNDANCES %%%%%%%%%%%%%
c = 2; % [gC m^-2] total abundance of copepods in the water column
f = 1; % [gC m^-2] total abundance of forage fish in the water column
m = 5; % [gC m^-2] total abundance of mesopelagic fish in the water column
...
%Top predator
P.EDAF = repmat(PDAF,1,P.n)*P.gamma*pi*P.uA.*P.MSDA.*(repmat(VisD(20,10^-3,P.lA),1,P.n).^2);%-repmat(VisDF(P.lA),1,P.n).^2); % [m^3 day^-1] clearance rate of top predator during daytime with visual feeding
...
P.EDAM = repmat(PDAM,1,P.n)*P.gamma*pi*P.uA.*P.MSDA.*(repmat(VisD(20,10^-6,P.lA),1,P.n).^2);%-repmat(VisDM(P.lA),1,P.n).^2); % [m^3 day^-1] clearance rate of top predator during daytime with visual feeding  ------  CHANGED HERE
% P.EDAM = max(P.EDAM, 0.5*pi*P.uA*P.MSDA*(0.1*P.lA/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.EDAB = repmat(PDAB,1,P.n)*P.gamma*pi*P.uA.*P.MSDA.*(repmat(VisD(50,10^-6,P.lA),1,P.n).^2);%-repmat(VisDB(P.lA),1,P.n).^2); % [m^3 day^-1] clearance rate of top predator during daytime with visual feeding
% P.EDAB = max(P.EDAB, 0.5*pi*P.uA*P.MSDA*(0.1*P.lA/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENAF = repmat(PNAF',P.n,1)*P.gamma*pi*P.uA.*P.MSNA.*(repmat(VisN(20,10^-3,P.lA),P.n,1).^2);%-repmat(VisNF(P.lA),P.n,1).^2); % [m^3 day^-1] clearance rate of top predator during nighttime with visual feeding
...
P.ENAM = repmat(PNAM',P.n,1)*P.gamma*pi*P.uA.*P.MSNA.*(repmat(VisN(20,10^-6,P.lA),P.n,1).^2);%-repmat(VisNM(P.lA),P.n,1).^2); % [m^3 day^-1] clearance rate of top predator during nighttime with visual feeding  ----- CHANGED HERE
% P.ENAM = max(P.ENAM, 0.5*pi*P.uA*P.MSNA*(0.1*P.lA/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENAB = repmat(PNAB',P.n,1)*P.gamma*pi*P.uA.*P.MSNA.*(repmat(VisN(50,10^-6,P.lA),P.n,1).^2);%-repmat(VisNB(P.lA),P.n,1).^2); % [m^3 day^-1] clearance rate of top predator during nighttime with visual feeding
...
P.EDMC = repmat(PDMC,1,P.n)*P.gamma*pi*P.uM.*P.MSDM.*(repmat(VisD(0.2,10^-8,P.lM),1,P.n).^2);%-P.RC.^2); % [m^3 day^-1] clearance rate of mesopelagic fish during daytime with visual feeding
% P.EDMC = max(P.EDMC, 0.5*pi*P.uM*P.MSDM*(P.lM/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENMd = repmat(PNMd',P.n,P.n)*P.gamma*pi*P.uM.*P.MSNM.*(repmat(VisN(0.2,10^-8,P.lM),P.n,1).^2-0); % [m^3 day^-1] clearance rate of mesopelagic fish during nighttime with visual feeding
% P.ENMd = max(P.ENMd, 0.5*pi*P.uM*P.MSNM*(P.lM/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENMC = repmat(PNMC',P.n,1)*P.gamma*pi*P.uM.*P.MSNM.*(repmat(VisN(0.2,10^-8,P.lM),P.n,1).^2);%-P.RC.^2); % [m^3 day^-1] clearance rate of mesopelagic fish during nighttime with visual feeding
% P.ENMC = max(P.ENMC, 0.5*pi*P.uM*P.MSNM*(P.lM/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

%Bathypelagic fish
P.EDBd = repmat(PDBd,P.n,P.n)*P.gamma*pi*P.uB.*P.MSDB.*(repmat(VisD(1,10^-25,P.lB),1,P.n).^2-0); % [m^3 day^-1] clearance rate of mesopelagic fish during daytime with visual feeding
% P.EDBd = max(P.EDBd, 0.5*pi*P.uB*P.MSDB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.EDBb = repmat(PDBb,P.n,P.n)*P.gamma*pi*P.uB.*P.MSDB.*(repmat(VisD(1,10^-25,P.lB),1,P.n).^2-0); % [m^3 day^-1] clearance rate of mesopelagic fish during daytime with visual feeding
%P.EDBb = max(P.EDBb, 0.5*pi*P.uB*P.MSDB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.EDBC = repmat(PDBC,1,P.n)*P.gamma*pi*P.uB.*P.MSDB.*(repmat(VisD(1,10^-25,P.lB),1,P.n).^2);%-P.RC.^2); % [m^3 day^-1] clearance rate of mesopelagic fish during daytime with visual feeding
%P.EDBC = max(P.EDBC, 0.5*pi*P.uB*P.MSDB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.EDBM = repmat(PDBM,1,P.n)*P.gamma*pi*P.uB.*P.MSDB.*(repmat(VisD(2,10^-25,P.lB),1,P.n).^2);%-repmat(VisDM(P.lB),1,P.n).^2); % [m^3 day^-1] clearance rate of mesopelagic fish during daytime with visual feeding
%P.EDBM = max(P.EDBM, 0.5*pi*P.uB*P.MSDB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENBd = repmat(PNBd',P.n,P.n)*P.gamma*pi*P.uB.*P.MSNB.*(repmat(VisN(1,10^-25,P.lB),P.n,1).^2-0); % [m^3 day^-1] clearance rate of mesopelagic fish during nighttime with visual feeding
%P.ENBd = max(P.ENBd, 0.5*pi*P.uB*P.MSNB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENBb = repmat(PNBb',P.n,P.n)*P.gamma*pi*P.uB.*P.MSNB.*(repmat(VisN(1,10^-25,P.lB),P.n,1).^2-0); % [m^3 day^-1] clearance rate of mesopelagic fish during nighttime with visual feeding
%P.ENBb = max(P.ENBb, 0.5*pi*P.uB*P.MSNB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENBC = repmat(PNBC',P.n,1)*P.gamma*pi*P.uB.*P.MSNB.*(repmat(VisN(1,10^-25,P.lB),P.n,1).^2);%-P.RC.^2); % [m^3 day^-1] clearance rate of mesopelagic fish during nighttime with visual feeding
%P.ENBC = max(P.ENBC, 0.5*pi*P.uB*P.MSNB*(P.lB/2)^2); % [m^3 day^-1] Clearance rate is the max of visual and filtering potentials

P.ENBM = repmat(PNBM',P.n,1)*P.gamma*pi*P.uB.*P.MSNB.*(repmat(VisN(2,10^-25,P.lB),P.n,1).^2);%-repmat(VisNM(P.lB),P.n,1).^2); % [m^3 day^-1] clearance rate of mesopelagic fish during nighttime with visual feeding
