...
P.n = 100; % [-] Number of water layers that we want
...
P.pO2 = 21*ones(size(P.zi)); %pO2func(P.zi); % %21*ones(size(P.zi)); %min(21,P.O2./(0.381*exp(5.7018*(28-P.T)./(P.T+273.15)))/0.75); % [kPa] Partial pressure of oxygen in the water column
...
%%%%%%%%%%% ABUNDANCES %%%%%%%%%%%%%
c = 4; %10; % [gC m^-2] total abundance of copepods in the water column 4
f = 2; %1; % [gC m^-2] total abundance of forage fish in the water column 2
...
b = 0.01; % [gC m^-2] total abundance of bathypelagic fish in the water column
j = 0.1; % [gC m^-2] total abundance of tactile predators in the water column
r = 10; % [gC m^-2] total abundance of phytoplankton in the water column
% d = 
benth = 10; % [gC m^-2] total abundance of benthos in the water column

P.R  = r*exp(-(P.zi-50).^2/30^2)/P.ZMAX / sum(exp(-(P.zi-50).^2/30^2)) ; %0.01*(1-tanh((P.zi-P.z0)/P.zm))/2; % [gC / m3] Resource concentration
...
%Top predator
P.EDAF = repmat(PDAF,1,P.n)*P.gamma*pi*P.uA.*P.MSDA.*(repmat(VisD(3,10^-6,P.lA),1,P.n).^2);%-repmat(VisDF(P.lA),1,P.n).^2); % [m^3 day^-1] clearance rate of top predator during daytime with visual feeding
...
P.EDAM = repmat(PDAM,1,P.n)*P.gamma*pi*P.uA.*P.MSDA.*(repmat(VisD(10,10^-3,P.lA),1,P.n).^2);%-repmat(VisDM(P.lA),1,P.n).^2); % [m^3 day^-1] clearance rate of top predator during daytime with visual feeding  ------  CHANGED HERE
...
P.ENAF = repmat(PNAF',P.n,1)*P.gamma*pi*P.uA.*P.MSNA.*(repmat(VisN(3,10^-6,P.lA),P.n,1).^2);%-repmat(VisNF(P.lA),P.n,1).^2); % [m^3 day^-1] clearance rate of top predator during nighttime with visual feeding
...
P.ENAM = repmat(PNAM',P.n,1)*P.gamma*pi*P.uA.*P.MSNA.*(repmat(VisN(10,10^-3,P.lA),P.n,1).^2);%-repmat(VisNM(P.lA),P.n,1).^2); % [m^3 day^-1] clearance rate of top predator during nighttime with visual feeding  ----- CHANGED HERE
