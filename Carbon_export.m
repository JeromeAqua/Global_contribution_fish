...
DegPOC = zeros(P.n+1 ,6);

DegPOC(1:end-1,1) = Dmean(:,2).*(1-exp(-P.alpha(:,2)*P.dZ/P.SR(2)))*P.SR(2); % *P.dZ/P.dZ [gC m^-2 day^-1] Degradation of the faecal pellets due to small copepods
DegPOC(1:end-1,2) = Dmean(:,3).*(1-exp(-P.alpha(:,3)*P.dZ/P.SR(3)))*P.SR(3); % [gC m^-2 day^-1] Degradation of the faecal pellets due to big copepods
DegPOC(1:end-1,3) = Dmean(:,4).*(1-exp(-P.alpha(:,4)*P.dZ/P.SR(4)))*P.SR(4); % [gC m^-2 day^-1] Degradation of the faecal pellets due to mesopelagic fish
DegPOC(1:end-1,4) = Dmean(:,5).*(1-exp(-P.alpha(:,5)*P.dZ/P.SR(5)))*P.SR(5); % [gC m^-2 day^-1] Degradation of the faecal pellets due to forage fish
DegPOC(1:end-1,5) = Dmean(:,6).*(1-exp(-P.alpha(:,6)*P.dZ/P.SR(6)))*P.SR(6); % [gC m^-2 day^-1] Degradation of the faecal pellets due to top predators
DegPOC(1:end-1,6) = Dmean(:,7).*(1-exp(-P.alpha(:,7)*P.dZ/P.SR(7)))*P.SR(7); % [gC m^-2 day^-1] Degradation of the faecal pellets due to jellyfish
