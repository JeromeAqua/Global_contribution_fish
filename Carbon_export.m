...
DIC(:,2) = P.SMRP'.*((1-P.sigma)*PPnight + P.sigma*PPday); % [gC m^-3 day^-1] Respiration by big copepods at each depth 
DIC(:,3) = P.SMRM'.*((1-P.sigma)*mmnight + P.sigma*mmday); % [gC m^-3 day^-1] Respiration by mesopelagic at each depth 
DIC(:,4) = P.SMRF'.*((1-P.sigma)*FFnight + P.sigma*FFday); % [gC m^-3 day^-1] Respiration by forage fish at each depth 
DIC(:,5) = P.SMRA'.*((1-P.sigma)*AAnight + P.sigma*AAday); % [gC m^-3 day^-1] Respiration by top predators at each depth 
DIC(:,6) = P.SMRJ'.*((1-P.sigma)*JJnight + P.sigma*JJday); % [gC m^-3 day^-1] Respiration by jellyfish at each depth 

DIC_depth = cumsum(DIC,1,'reverse')*P.dZ; % [gC m^-2 day^-1] Respiration of organisms below each depth
