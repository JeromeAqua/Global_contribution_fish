...
SDA_F = SDA_C; EatF = SDA_C; RespiF = SDA_C; PredaF = SDA_C; BcgF = SDA_C;
...
%             P.J = 0.1; %no need to have J as it does not change
            ...
            PredaC(j,i) =  Mpc(j,i)*P.fPR + Mfc(j,i)*P.fF + Mmc(j,i)*P.fMC + Mjc(j,i)*P.fJ;
            ...
            PredaP(j,i) = Mmp(j,i)*P.fMC + Mfp(j,i)*P.fF + Mjp(j,i)*P.fJ;
            BcgP(j,i) = sum(P.dZ*P.n*P.P*(P.sigma*sum((minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_P(j,i,:,:)),2)+(1-P.sigma)*sum((minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_P(j,i,:,:)),1)'));
            SDA_P(j,i) = Mpd(j,i)*P.fPd + Mpr(j,i)*P.fPR + Mpc(j,i)*P.fPR - sum(squeeze(DIC_glob(j,i,:,2))) -...
                         Mmp(j,i)*P.fMC - Mfp(j,i)*P.fF - Mjp(j,i)*P.fJ -...
                         sum(P.dZ*P.n*P.P*(P.sigma*sum((minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_P(j,i,:,:)),2)+(1-P.sigma)*sum((minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_P(j,i,:,:)),1)'));
            
            EatM(j,i) = Mmc(j,i)*P.fMC + Mmp(j,i)*P.fMC;
            RespiM(j,i) = sum(squeeze(DIC_glob(j,i,:,3)));
            PredaM(j,i) =  Mfm(j,i)*P.fF + Mam(j,i)*P.fA;
            BcgM(j,i) = sum(P.dZ*P.n*P.M*(P.sigma*sum((0.01 +minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_M(j,i,:,:)),2)+(1-P.sigma)*sum((0.01+minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_M(j,i,:,:)),1)'));

            SDA_M(j,i) = Mmc(j,i)*P.fMC + Mmp(j,i)*P.fMC - sum(squeeze(DIC_glob(j,i,:,3))) -...
                         Mfm(j,i)*P.fF - Mam(j,i)*P.fA -...
                         sum(P.dZ*P.n*P.M*(P.sigma*sum((0.01 +minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_M(j,i,:,:)),2)+(1-P.sigma)*sum((0.01+minimort+1*(P.LD'+P.LN)/max(max(P.LD'+P.LN))).*squeeze(Glob_M(j,i,:,:)),1)'));
            
            EatF(j,i) =  Mfc(j,i)*P.fF + Mfp(j,i)*P.fF + Mfm(j,i)*P.fF;
            RespiF(j,i) = sum(squeeze(DIC_glob(j,i,:,4)));
            PredaF(j,i) = Maf(j,i)*P.fA;
            ...
            BcgA(j,i) = sum(P.dZ*P.n*P.A*(P.sigma*sum(minimort.*squeeze(Glob_A(j,i,:,:)),2)+(1-P.sigma)*sum(minimort.*squeeze(Glob_A(j,i,:,:)),1)'));
            SDA_A(j,i) = Maf(j,i)*P.fA + Mam(j,i)*P.fA + Maj(j,i)*P.fA - sum(squeeze(DIC_glob(j,i,:,5))) -...
                         sum(P.dZ*P.n*P.A*(P.sigma*sum(minimort.*squeeze(Glob_A(j,i,:,:)),2)+(1-P.sigma)*sum(minimort.*squeeze(Glob_A(j,i,:,:)),1)'));
            
            EatJ(j,i) = Mjc(j,i)*P.fJ + Mjp(j,i)*P.fJ;
            RespiJ(j,i) = sum(squeeze(DIC_glob(j,i,:,6)));
            PredaJ(j,i) = Maj(j,i)*P.fA;
